...
export default async function ParentAttendancePage() {
    ...
    return (
      <div className="h-full flex flex-col gap-3">
        <div className="shrink-0" style={{ paddingTop: 23 }}>
          <h1 className="text-[22px] font-semibold text-[#2d2d2d] dark:text-white">Attendance</h1>
        </div>
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      <div className="shrink-0" style={{ paddingTop: 23 }}>
        <p className="text-[12px] text-[#9A9A9A] dark:text-[#A0A0A0]">{child?.full_name || "Your child"} · Last 60 days</p>
      ...
      {rate !== null && (
        <div className="dash-card dark:border-[#2D2D2D] bg-white dark:bg-[#333333] rounded-2xl px-5 py-4 shrink-0">
          ...
          <p className="text-[11px] text-[#9A9A9A] dark:text-[#A0A0A0] mt-1.5">
            {rate >= 90 ? "Excellent attendance" : rate >= 75 ? "Good — could improve to above 90%" : "Below target — please contact the school"}
          </p>
        </div>
      )}
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {attendance.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center px-5">
              ...
              <p className="text-[12px] text-[#9A9A9A] mt-1">Attendance will appear here once the teacher marks the register</p>
            </div>
          ) : (
